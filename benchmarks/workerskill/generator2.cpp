...
const int TEST_SHIFT = 10;
const int TEST_NUMBER = 10;

int main() {	
	for (int i = TEST_SHIFT; i < TEST_SHIFT+TEST_NUMBER; i++) {
		char filename[5 + ((i+1)/10)];
		sprintf(filename,"%d%s",i+1, ".hex");
		freopen(filename, "w", stdout);

		cout << endl << "$dom(0.." << (i+5) << ")." << endl;

		int task_number = 2 + i - TEST_SHIFT + ((rand() + 1 ) % 3), worker_number = 2 + i - TEST_SHIFT;
